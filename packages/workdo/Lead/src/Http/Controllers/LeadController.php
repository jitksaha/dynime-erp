...
use Workdo\Lead\Models\LeadNote;
...
class LeadController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-leads')) {
            ...
            $leads = Lead::with(['stage', 'user', 'userLeads.user', 'leadNotes.creator:id,name'])
    ...
    public function store(StoreLeadRequest $request)
    {
        if (Auth::user()->can('create-leads')) {
            ...
            if (empty($stage)) {
                ...
                $lead->project_value  = $request->project_value ?? 0;
    ...
    public function convertToDeal(ConvertToDealRequest $request, Lead $lead)
    {
        if (Auth::user()->can('edit-leads')) {
            ...
            if ($request->client_check == 'exist') {
            ...
            } elseif ($request->client_check == 'new') {
                ...
                if (empty($clientEmail) && !empty($request->client_phone)) {
                    $cleanPhone = preg_replace('/[^0-9]/', '', $request->client_phone);
                    $clientEmail = $cleanPhone . '@dynime-client.com';
                }

                $client = User::create([
                    'name' => $request->client_name,
                    'email' => $clientEmail,
                    'mobile_no' => $request->client_phone,
                ...
                if (company_setting('New User') == 'on' && !empty($request->client_email)) {
                    $emailData = [
                        'name' => $request->client_name,
            ...
            if (company_setting('Deal Assign') == 'on' && $client) {
                $emailData = [
                    'deal_name'     => $deal->name,
    ...
    public function getNotes(Lead $lead)
    {
        if (Auth::user()->can('edit-leads') && $lead->created_by == creatorId()) {
            $notes = LeadNote::with('creator:id,name')
    ...
    public function storeNote(Request $request, Lead $lead)
    {
        if (Auth::user()->can('edit-leads') && $lead->created_by == creatorId()) {
            ...
            return response()->json(['success' => true, 'message' => __('Note added successfully.'), 'note' => $note]);
        }
        return response()->json(['error' => __('Permission denied')], 403);
    }

    public function destroyNote($leadId, $noteId)
    {
        $note = LeadNote::where('lead_id', $leadId)->where('id', $noteId)->first();
        if ($note && Auth::user()->can('edit-leads') && $note->lead->created_by == creatorId()) {
            $note->delete();
            return response()->json(['success' => true, 'message' => __('Note deleted successfully.')]);
        }
        return response()->json(['error' => __('Permission denied')], 403);
    }
}
