...
import { Eye, Trash2, Plus, Calculator, DollarSign, TrendingUp, ShieldCheck, Check } from 'lucide-react';
...
export default function Edit() {
    const { employee = {}, users = [], allRoles = [], managers = [], branches = [], departments = [], designations = [], shifts = [], existingDocuments = [], documentTypes = [], allowanceTypes = [], deductionTypes = [], existingAllowances = [], existingDeductions = [], shiftHistory = [], companyAllSetting = {} } = usePage<any>().props;
    ...
    const { data, setData, put, processing, errors, transform, clearErrors } = useForm<EditEmployeeFormData>({
        ...
        roles_responsibilities: employee.roles_responsibilities || '',
        mobile_no: employee.user?.mobile_no ?? '',
        avatar: employee.user?.avatar || null,
        date_of_birth: employee.date_of_birth || '',
        gender: employee.gender || 'Male',
        shift_id: employee.shift?.toString() || '',
        is_flexible_shift_allowed: Boolean(employee.is_flexible_shift_allowed),
        ...
        additional_branch_ids: employee.additional_branch_ids || [],
        roles: employee.user?.roles?.map((r: any) => r.name) || [],
                            ...
                            <TabsContent value="employment" className="space-y-6 mt-6">
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        {/* Shift Effective From Date */}
                                        <div className="mt-2.5">
                                            <Label className="text-xs text-slate-700">{t('Shift Effective Date')}</Label>
                                            <DatePicker
                                                value={(data as any).shift_effective_from || new Date().toISOString().split('T')[0]}
                                                onChange={(date) => setData('shift_effective_from' as any, date)}
                                                placeholder={t('Effective From Date')}
                                            />
                                        </div>

                                        <div className="flex items-center space-x-2 pt-2">
                                            <Checkbox
                                                id="is_flexible_shift_allowed"
                                                checked={Boolean(data.is_flexible_shift_allowed)}
                                                onCheckedChange={(checked) => setData('is_flexible_shift_allowed', Boolean(checked))}
                                            />
                                            <Label htmlFor="is_flexible_shift_allowed" className="text-xs font-bold text-slate-800 cursor-pointer">
                                                {t('Allow Flexible Shift Access')}
                                            </Label>
                                        </div>
                                        ...
                                        {shiftHistory && shiftHistory.length > 0 && (
                                            <div className="mt-3 p-3 rounded-xl bg-slate-50 border border-slate-200/80 space-y-2">
                                                ...
                                                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                                                    {shiftHistory.map((hist: any) => (
                                                        <div key={hist.id} className="p-2 rounded-lg bg-white border border-slate-200/90 text-[11px] flex items-center justify-between">
                                                            <div>
                                                                <span className="font-bold text-slate-900">{hist.shift?.shift_name || ('Shift #' + hist.shift_id)}</span>
                                                                <span className="ml-1 text-slate-500 font-mono">v{hist.shift_version || 1}</span>
                                                            </div>
                                                            <div className="text-slate-600 font-mono text-[10.5px]">
                                                                {hist.effective_from} → {hist.effective_to || 'Present'}
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                    ...
                                    <div className="md:col-span-2 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-3">
                                         <div>
                                             ...
                                             <p className="text-xs text-slate-500 mt-0.5">
                                                 {t('Permissions are inherited from all assigned roles (e.g. Employee, HR, Manager, Finance, Admin).')}
                                             </p>
                                         </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 border-t border-slate-200 dark:border-slate-800 pt-6 mt-6">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {(data.allowances || []).map((allowance: any, index: number) => (
                                                <div key={index} className="grid grid-cols-12 gap-2 items-end p-3 border border-slate-200 dark:border-slate-800 rounded-lg bg-slate-50/50 dark:bg-slate-900/50">
                                                    <div className="col-span-5 space-y-1">
                                                        <Label className="text-xs">{t('Allowance Type')}</Label>
                                                        <Select
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {(data.deductions || []).map((deduction: any, index: number) => (
                                                <div key={index} className="grid grid-cols-12 gap-2 items-end p-3 border border-slate-200 dark:border-slate-800 rounded-lg bg-slate-50/50 dark:bg-slate-900/50">
                                                    <div className="col-span-5 space-y-1">
                                                        <Label className="text-xs">{t('Deduction Type')}</Label>
                                                        <Select
