...
import { Package, Plus, Power, PowerOff, Eye } from "lucide-react";
...
export default function AddonSettings() {
    const { modules = [], auth } = usePage().props as any;
    ...
    return (
        <Card className="border border-gray-200 shadow-sm rounded-xl">
            <CardHeader className="border-b border-gray-100 pb-4 flex flex-row items-center justify-between">
                <div>
                    <h2 className="text-lg font-semibold text-gray-900">{t('Add-ons Manager')}</h2>
                    <p className="text-xs text-gray-500">{t('Manage and configure system add-ons and modules')}</p>
                </div>
                <TooltipProvider>
                ...
                {filteredModules.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filteredModules.map((module: Module) => (
                            <Card key={module.name} className="relative hover:shadow-md transition-all duration-200 border border-gray-200 flex flex-col rounded-lg">
